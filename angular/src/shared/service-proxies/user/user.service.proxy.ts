...
import { CreateUserDto, UserDto, ChangePasswordDto, ChangeUserLanguageDto, UserDtoPagedResultDto, ResetPasswordDto, UserPagedResultDto } from './dto/user-dto';
import { RoleDtoListResultDto } from '../role/dto/role-dto'
import { ApiException } from '../api-exception';
import { FlatPermissionDto, PermissionDtoListResultDto } from '../permission/dto/permission-dto';
...
export class UserServiceProxy {
    ...
    getUsers(): Observable<UserPagedResultDto> {
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            ...
            if (response_ instanceof HttpResponseBase) {
                try {
                    ...
                    return <Observable<UserPagedResultDto>><any>_observableThrow(e);
                }
            } else
                return <Observable<UserPagedResultDto>><any>_observableThrow(response_);
        }));
    }

    protected processGetUsers(response: HttpResponseBase): Observable<UserPagedResultDto> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
        ...
        if (status === 200) {
            ...
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = UserPagedResultDto.fromJS(resultData200);
            return _observableOf(result200);
            }));
        } else if (status !== 200 && status !== 204) {
            return blobToText(responseBlob).pipe(_observableMergeMap(_responseText => {
        ...
        return _observableOf<UserPagedResultDto>(<any>null);
    }
}
