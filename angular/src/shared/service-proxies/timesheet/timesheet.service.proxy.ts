...
export class TimeSheetServiceProxy {
    ...
    getCourses(): Observable<CourseDtoListResultDto> {
        let url_ = this.baseUrl + "/api/services/app/TimeSheetEntry/GetCourseWithSubject";
        url_ = url_.replace(/[?&]$/, "");
    ...
    getAllTimeSheetFromDateToDate(fromDate: Date | undefined, toDate: Date | undefined): Observable<TimeSheetDtoPagedResultDto> {
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGetAllTimeSheetFromDateToDate(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetAllTimeSheetFromDateToDate(<any>response_);
                } catch (e) {
                    return <Observable<TimeSheetDtoPagedResultDto>><any>_observableThrow(e);
                }
            } else
    ...
    protected processGetAllTimeSheetFromDateToDate(response: HttpResponseBase): Observable<TimeSheetDtoPagedResultDto> {
        const status = response.status;
        const responseBlob = 
            response instanceof HttpResponse ? response.body : 
            (<any>response).error instanceof Blob ? (<any>response).error : undefined;
